...
public class AutoShootWithIndex extends CommandBase {
   ...
   */
  public AutoShootWithIndex(ShooterSubsystem shooter) {
    m_shooter = shooter;
  ...
  public void initialize() {
      timer.reset();
      timer.start();
      m_shooter.shoot(AutoConstants.kAutoShootPower);
  }
 
  //this waits until the shooter is charged up, then launches the balls
  @Override
  public void execute() {
      if(timer.get() >= AutoConstants.kAutoChargeUpTime)
      {
